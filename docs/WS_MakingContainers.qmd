...
This will create a new container, **myubuntu.sif** from an existing container from DockerHub.

Typically, SIF formatted containers can **NOT** be modified once you create them. 

- Sandbox directories
  - Create a container within a writable directory (Sandbox) 
  - Great to expermint installing software
  
apptainer build --sandbox myubuntu-SB docker://ubuntu:22.04
...
Once we have a Sandbox directory image, we can go inside the directory and make changes within the container with the **--writable** flag

apptainer shell --writable mybuntu-SB

Any changes made will be persistent when exiting the container.

Once you make you changes, you can then convert the Sandbox container to a SIF container

apptainer build myNewUbuntu.sif myubuntu-SB

This will create a new SIF (single file) container, **myNewUbuntu.sif** from the Sandbox directory **myubuntu-SB**
...
- `%runscript` - define command to run when you enter the container
  - for when running `apptainer run`
...
apptainer exec lolcow.sif cowsay

Execute the runscript from the container
...
```{.bash}
apptainer build pytorch.sif ubuntu_22.04_SB/
```
...
- Instead of installing these dependencies on H2 (or loading modules)
...
# Example 3: Miniconda { style="font-size: 0.75em"}

Miniconda is a very popular python and R distributaion for simplifying package installation 
...
Though Miniconda can be tricky installing in a container due to environment setup.
...
In this example, we will use Miniconda to install h2o packages inside
python and R.
...
-   Install Miniconda from an env.yml
-   Use of `%runscript` to setup Miniconda env for `apptainer run`
